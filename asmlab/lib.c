#include "lib.h"

funcCmp_t *getCompareFunction(type_t t)
{
    switch (t)
    {
    case TypeInt:
        return (funcCmp_t *)&intCmp;
        break;
    case TypeString:
        return (funcCmp_t *)&strCmp;
        break;
    case TypeCard:
        return (funcCmp_t *)&cardCmp;
        break;
    default:
        break;
    }
    return 0;
}
funcClone_t *getCloneFunction(type_t t)
{
    switch (t)
    {
    case TypeInt:
        return (funcClone_t *)&intClone;
        break;
    case TypeString:
        return (funcClone_t *)&strClone;
        break;
    case TypeCard:
        return (funcClone_t *)&cardClone;
        break;
    default:
        break;
    }
    return 0;
}
funcDelete_t *getDeleteFunction(type_t t)
{
    switch (t)
    {
    case TypeInt:
        return (funcDelete_t *)&intDelete;
        break;
    case TypeString:
        return (funcDelete_t *)&strDelete;
        break;
    case TypeCard:
        return (funcDelete_t *)&cardDelete;
        break;
    default:
        break;
    }
    return 0;
}
funcPrint_t *getPrintFunction(type_t t)
{
    switch (t)
    {
    case TypeInt:
        return (funcPrint_t *)&intPrint;
        break;
    case TypeString:
        return (funcPrint_t *)&strPrint;
        break;
    case TypeCard:
        return (funcPrint_t *)&cardPrint;
        break;
    default:
        break;
    }
    return 0;
}

/** Int **/

int32_t intCmp(int32_t *a, int32_t *b)
{
    if (*a < *b) {
        return 1;
    } else if (*b < *a) {
        return -1;
    } else {
        return 0;
    }
}

void intDelete(int32_t *a) {
    free(a);
}

void intPrint(int32_t *a, FILE *pFile)
{
    if (a != NULL && pFile !=NULL){
        fprintf(pFile, "%d", *a);
    }
}

int32_t *intClone(int32_t *a)
{
    int32_t *b = malloc(sizeof(int32_t));
    *b = *a;
    return b;
}

/** Lista **/

list_t *listNew(type_t t) {
    list_t *list = malloc(sizeof(list_t));
    if (list == NULL){
        return NULL;
    }
    list->type = t;
    list->size = 0;
    list->first = NULL;
    list->last = NULL;
    return list;
}

uint8_t listGetSize(list_t *l)
{
    if(l == NULL){
        return 0;
    }
    return l->size;
}

void *listGet(list_t *l, uint8_t i)
{
    if (l->first == NULL) {
        return NULL;
    }
    if (i >= l->size) {
        return NULL;
    }
    listElem_t *actual = l->first;
    for (int j = 0; j < i; j++) {
        actual = actual->next;
    }
    return actual->data;
}

void listAddFirst(list_t *l, void *data){
    listElem_t *elem = malloc(sizeof(listElem_t));
    if (elem == NULL) {
        return;
    }
    elem->data = (getCloneFunction(l->type))(data);
    elem->next = l->first;
    elem->prev = NULL;
    if (l->first != NULL) {
        l->first->prev = elem;
    } else {
        l->last = elem;
    }
    l->first = elem;
    l->size++;
}

void listAddLast(list_t *l, void *data)
{
    if(l == NULL){
        return;
    }

    listElem_t* nuevo_elemento = malloc (sizeof(listElem_t));
    if (nuevo_elemento == NULL){
        printf("Error de memoria\n");
        return;
    }

    nuevo_elemento->data = (getCloneFunction(l->type))(data);
    nuevo_elemento->next = NULL;

    if (l->first == NULL){
        nuevo_elemento->prev = NULL;
        l->first = nuevo_elemento;
        l->last = nuevo_elemento;
    } else {
        nuevo_elemento->prev=l->last;
        l->last->next = nuevo_elemento;
        l->last = nuevo_elemento;
    }
    l->size++;
}

list_t *listClone(list_t *l)
{
    list_t *lista_copia = listNew(l->type);
    lista_copia->size = l->size;
    funcClone_t *clone = getCloneFunction(l->type);
    if (l->first == NULL) {
        lista_copia->first = NULL;
        lista_copia->last = NULL;
        return lista_copia;
    }

    listElem_t *actual_lista = l->first;
    listElem_t *prev = NULL;

    while (actual_lista != NULL) {
        listElem_t *nodo = malloc(sizeof(listElem_t));
        nodo->prev = prev;
        if (lista_copia->first == NULL) {
            lista_copia->first = nodo;
        } else {
            nodo->prev->next = nodo;
        }
        nodo->data = clone(actual_lista->data);
        nodo->next = NULL;

        prev = nodo;
        actual_lista = actual_lista->next;

    }
    lista_copia->last = prev;
    return lista_copia;
}

void *listRemove(list_t *l, uint8_t i){
    if (i >= l->size) {
        return 0;
    }
    listElem_t* elem = l->first;
    for (int j = 0; j < i; j++) {
        elem = elem->next;
    }
    if (elem == NULL){
        return NULL;
    }
    void* data = elem->data;
    if (elem->prev != NULL){
        elem->prev->next = elem->next;
    } else {
        l->first = elem->next;
    }
    if (elem->next != NULL) {
        elem->next->prev = elem->prev;
    } else {
        l->last = elem->prev;
    }
    free(elem);
    l->size--;
    return data;
}

void listSwap(list_t *l, uint8_t i, uint8_t j)
{
    if(l == NULL){
        return;
    }
    
    if (i >= l->size || j >= l->size || i == j){
        return;
    }

    listElem_t* elemento_i = l->first;
    listElem_t* elemento_j = l->first;
    
    for (uint8_t k = 0; k < i; k++){
        elemento_i = elemento_i->next;
    }
    for (uint8_t k = 0; k < j; k++){
        elemento_j = elemento_j->next;
    }

    void* data_aux = elemento_i->data;
    elemento_i->data = elemento_j->data;
    elemento_j->data = data_aux;
}

void listDelete(list_t *l)
{
    if (l->first == NULL) {
        free(l);
        return;
    }
    funcDelete_t *delete = getDeleteFunction(l->type);
    listElem_t *actual = l->first;
    while (actual != NULL) {
        listElem_t *copia_actual = actual;
        copia_actual = actual;
        actual = actual->next;
        if (copia_actual->data != NULL) {
            delete(copia_actual->data);
        }
        free(copia_actual);
    }
    free(l);
}

void listPrint(list_t *l, FILE *pFile){
    fprintf(pFile, "[");
    listElem_t* elem = l->first;
    for (int j = 0; j < l->size && elem != NULL ; j++) {
        (getPrintFunction(l->type))(elem->data, pFile);
        elem = elem->next;
        if (j < l->size - 1){
            fprintf(pFile, ",");
        }
    }
    fprintf(pFile, "]");
}

/** Game **/

game_t *gameNew(void *cardDeck, funcGet_t *funcGet, funcRemove_t *funcRemove, funcSize_t *funcSize, funcPrint_t *funcPrint, funcDelete_t *funcDelete)
{
    game_t *game = (game_t *)malloc(sizeof(game_t));
    game->cardDeck = cardDeck;
    game->funcGet = funcGet;
    game->funcRemove = funcRemove;
    game->funcSize = funcSize;
    game->funcPrint = funcPrint;
    game->funcDelete = funcDelete;
    return game;
}
int gamePlayStep(game_t *g)
{
    int applied = 0;
    uint8_t i = 0;
    while (applied == 0 && i + 2 < g->funcSize(g->cardDeck))
    {
        card_t *a = g->funcGet(g->cardDeck, i);
        card_t *b = g->funcGet(g->cardDeck, i + 1);
        card_t *c = g->funcGet(g->cardDeck, i + 2);
        if (strCmp(cardGetSuit(a), cardGetSuit(c)) == 0 || intCmp(cardGetNumber(a), cardGetNumber(c)) == 0)
        {
            card_t *removed = g->funcRemove(g->cardDeck, i);
            cardAddStacked(b, removed);
            cardDelete(removed);
            applied = 1;
        }
        i++;
    }
    return applied;
}
uint8_t gameGetCardDeckSize(game_t *g)
{
    return g->funcSize(g->cardDeck);
}
void gameDelete(game_t *g)
{
    g->funcDelete(g->cardDeck);
    free(g);
}
void gamePrint(game_t *g, FILE *pFile)
{
    g->funcPrint(g->cardDeck, pFile);
}
